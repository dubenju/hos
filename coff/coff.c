...
void display_coff_header(PFILEHDR pCoffHeader) {
    printf("print the information of the COFF header文件头.\n\n");
    ...
    printf("Characteristics(usFlags) =0X%04X\n", pCoffHeader->usFlags);
    ...
    if (pCoffHeader->usFlags & 0x0004) {
        printf("Section Flag             =F_LNNO(Line No is removed.).\n");
    }
...
void display_optional_header(POPTHDR pOpHeader) {
    printf("print the information of the optional header可选头.\n\n");
...
void display_section_header(PSECHDR pSecHeader, unsigned short idx) {
    ...
    printf("print the information of the section header段落头.\n\n");
...
void display_relocation_entry(PRELOC pRelEntry, unsigned long idx) {
    printf("print the information of the relocation entry.\n\n");
...
void display_line_no(PLINENO pLineNo, unsigned long idx) {
    printf("print the information of the line no.\n\n");
    printf("%d Line No ----------\n", idx);
    printf("Addr of Symbol =%04d\n", pLineNo->ulAddrORSymbol);
    printf("Line No  =04d\n", pLineNo->usLineNo);
}
...
void display_symbol_table_entry(PSYMENT pSymTblEnt, unsigned long idx) {
    ...
    printf("print the information of the symbol table.\n\n");
...
void display_string_table(char * buf, unsigned long size) {
    unsigned long idx = 1;
    unsigned long cnt = 0;
    printf("print the information of the string table.\n\n");
...
//    return 0;
    ...
    if (coff_header.usOptHdrSZ > 0) {
        ...
        printf("optional header not exist.\n\n");
    }
    ...
    while(section_header_cnd < coff_header.usNumSec) {
        ...
        if (section_header.ulSize != 0) {
            cur_pos_file = ftell(fp_in_file);
            fseek(fp_in_file, section_header.ulSecOffset, SEEK_SET);
            section_data = (char *) malloc(section_header.ulSize);
            if (section_data == NULL) {
                goto err_main;
            }
            memset(section_data, 0, section_header.ulSize);
            read_size = fread(section_data, sizeof(char), section_header.ulSize, fp_in_file);
            if (read_size < section_header.ulSize) {
                printf("read coff's optional header error.%d,%d\n", read_size, section_header.ulSize);
                goto err_main;
            } else {
                print_ary(section_data, section_header.ulSize);
                printf("\n");
            }
            if (section_data != NULL) {
                free(section_data);
    ...
    /* Line Number Entries 行号表 */
    long line_num_table_size = coff_header.ulSymbolOffset - ftell(fp_in_file);
    long line_num_readed_byte = 0;
    printf("sizeof(LINENO)=%d.\n", sizeof(LINENO));
    printf("line_num_table_size=%ld.\n", line_num_table_size);
    while(line_num_readed_byte < line_num_table_size) {
        memset(&line_no, 0, sizeof(LINENO));
        read_size = fread(&line_no, sizeof(char), sizeof(LINENO), fp_in_file);
        if (read_size < sizeof(LINENO)) {
            printf("read coff's line number error.%d,%d\n", read_size, sizeof(LINENO));
            goto err_main;
        } else {
            display_line_no(&line_no, (line_num_readed_byte / sizeof(LINENO)) + 1);
        }
        line_num_readed_byte += read_size;
    }
    if (line_num_table_size == 0) {
        printf("line no not exist.\n\n");
    }
    ...
    long cur_pos = ftell(fp_in_file);
    printf("postion:%d.\n", cur_pos);
    printf("String Table Size is :%d(file).\n", (file_size - cur_pos));

    long str_tbl_size = file_size - string_table_offset + 1;
