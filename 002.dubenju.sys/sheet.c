      ...
      if ((sht->vx0 & 2) == 0) {
        ...
        for (by = by0; by < by1; by++) {
          ...
          for (bx = bx0; bx < bx1 && (bx & 1) != 0; bx++) {	/* 前の端数を1バイトずつ */
            vx = sht->vx0 + bx;
            if (map[vy * ctl->xsize + vx] == sid) {
              *(unsigned short *) &vram[(vy * ctl->xsize + vx) * 2] = buf[by * sht->bxsize + bx];
            }
          }
          vx = sht->vx0 + bx;
          ...
          for (i = 0; i < i1; i++) {							/* 2の倍数部分 */
            if (o[i] == sid2) {
              ...
              if (map[vy * ctl->xsize + vx + 0] == sid) {
                *(unsigned short *) &vram[(vy * ctl->xsize + vx + 0) * 2] = buf[by * sht->bxsize + bx2 + 0];
              }
              if (map[vy * ctl->xsize + vx + 1] == sid) {
                *(unsigned short *) &vram[(vy * ctl->xsize + vx + 1) * 2] = buf[by * sht->bxsize + bx2 + 1];
              }
            }
          }
